...
try:from user_agent import generate_user_agent as H
...
try:import cfonts as I;from cfonts import render
except:C.system('pip install python-cfonts')
import random as J
def K():return J.choice(['cyan','blue','green','yellow','magenta'])
def L(C,D=.05):
...
def S(sessionid,url):
	...
	try:
		...
		if C.status_code==200:
			H=C.content;I={'profile_pic':('profile.jpg',H,'image/jpeg')};E=B.post('https://www.instagram.com/api/v1/web/accounts/web_change_profile_picture/',files=I)
	...
	except Exception as J:A(f"[!] Error: {J}")
def T():
	A('Choose login method:');A('[1] Login via sessionid');A('[2] Login via username and password');C=E('[+] Choose Your Login Method: ').strip()
	if C=='1':B=E('[+] Enter your Instagram sessionid: ').strip();D=Q(B)
	elif C=='2':D=E('[+] Enter your Instagram username: ').strip();G=E('[+] Enter your Instagram password: ').strip();B=P(D,G)
